...
subtitle: Data Carpentry "R for Social Scientists"
...
interviews_wide <- readRDS(file = "data/interviews_wide.rds")
interviews_long <- readRDS(file = "data/interviews_long.rds")
interviews_items_owned <- readRDS(file = "data/interviews_items_owned.rds")
interviews_items_owned_NA <- readRDS(file = "data/interviews_items_owned_NA.rds")
interviews_months_lack_food <- readRDS(file = "data/interviews_months_lack_food.rds")

#
# to load local version
#interviews <- read_csv("tutorials/r-socialsci/data/SAFI_clean.csv", na = "NULL")
...
<!-- This tutorial was made using the R package {learnr} https://rstudio.github.io/learnr/
     ...
     Modifications are licensed under CC-BY 4.0 by Martin Monkman 
...
Along with a number of other packages, **`dplyr`** and **`tidyr`** are part of the tidyverse. These packages are loaded in R's memory when we call them individually using `library(dplyr)` and `library(tidyr)`, or we can load them along with the other packages in the tidyverse: `library(tidyverse)`.
...
```{r, summarize01_example, exercise=TRUE, exercise.eval=FALSE}
interviews %>%
    group_by(village) %>%
...
```{r, summarize02_example, exercise=TRUE, exercise.eval=FALSE}
interviews %>%
...
```{r, summarize03_example, exercise=TRUE, exercise.eval=FALSE}
interviews %>%
    filter(!is.na(memb_assoc)) %>%
    group_by(village, memb_assoc) %>%
    summarize(mean_no_membrs = mean(no_membrs))
...
```{r, summarize04_example, exercise=TRUE, exercise.eval=FALSE}
interviews %>%
    filter(!is.na(memb_assoc)) %>%
    group_by(village, memb_assoc) %>%
    summarize(mean_no_membrs = mean(no_membrs),
...
```{r, summarize05_example, exercise=TRUE, exercise.eval=FALSE}
interviews %>%
    filter(!is.na(memb_assoc)) %>%
    group_by(village, memb_assoc) %>%
...
```{r, summarize06_example, exercise=TRUE, exercise.eval=FALSE}
interviews %>%
    filter(!is.na(memb_assoc)) %>%
    group_by(village, memb_assoc) %>%
    summarize(mean_no_membrs = mean(no_membrs),
...
```{r, counting01_example, exercise=TRUE, exercise.eval=FALSE}
interviews %>%
    count(village)
```
...
```{r, counting02_example, exercise=TRUE, exercise.eval=FALSE}
interviews %>%
...
```{r data_manip_ex3_1, exercise=TRUE, exercise.eval=TRUE}
...
```{r data_manip_ex3_2, exercise=TRUE, exercise.eval=TRUE}
...
Note: this will require you to use functions from the package **lubridate**. The 
reference pages for that package are here: [lubridate.tidyverse.org/](https://lubridate.tidyverse.org/)


```{r data_manip_ex3_3, exercise=TRUE, exercise.eval=TRUE}
...
```{r}
knitr::include_graphics("images/pivot_long_to_wide.png")
```
...
```{r pivot_01, exercise=TRUE, exercise.eval=TRUE}
...
<!--
#saveRDS(interviews_wide, file = "tutorials/r-socialsci/data/interviews_wide.rds")
#interviews_wide <- readRDS(file = "data/interviews_wide.rds")
-->
...
```{r pivot_02, exercise=TRUE, exercise.eval=TRUE}
...
```{r}
knitr::include_graphics("images/pivot_wide_to_long.png")
```
...
```{r pivot_03, exercise=TRUE, exercise.eval=TRUE}
interviews_long <- interviews_wide %>%
    pivot_longer(cols = c(burntbricks, cement, muddaub, sunbricks),
                 names_to = "respondent_wall_type", 
                 values_to = "wall_type_logical") %>%
...
<!--
#saveRDS(interviews_long, file = "tutorials/r-socialsci/data/interviews_long.rds")
#interviews_long <- readRDS(file = "data/interviews_long.rds")
-->
...
functions to use to fix a problem with the way that our data is structured. You may
have already learned that it's best practice to have only a
...
```{r pivot_04, exercise=TRUE, exercise.eval=TRUE}
...
<!--
#saveRDS(interviews_items_owned, file = "tutorials/r-socialsci/data/interviews_items_owned.rds")
#interviews_items_owned <- readRDS(file = "data/interviews_items_owned.rds")
-->
...
```{r pivot_05, exercise=TRUE, exercise.eval=TRUE}
interviews_items_owned_NA <- interviews_items_owned %>%
...
<!--
#saveRDS(interviews_items_owned_NA, file = "tutorials/r-socialsci/data/interviews_items_owned_NA.rds")
#interviews_items_owned_NA <- readRDS(file = "data/interviews_items_owned_NA.rds")
-->
...
```{r pivot_06, exercise=TRUE, exercise.eval=TRUE}
interviews_items_owned_NA %>%
    filter(bicycle) %>%
    group_by(village) %>%
    count(bicycle)
```
...
```{r pivot_07, exercise=TRUE, exercise.eval=TRUE}
interviews_items_owned_NA %>%
...
```{r data_manip_ex4_1, exercise=TRUE, exercise.eval=TRUE}
...
```{r data_manip_ex4_2, exercise=TRUE, exercise.eval=TRUE}
...
This tutorial is a modification of the material developed by the Data Carpentries lesson
["R for Social Scientists"](https://datacarpentry.org/r-socialsci/)


The source code for this tutorial can be found on GitHub at the [tutormanner repository](https://github.com/MonkmanMH/tutormanner/tree/master/tutorials/r-socialsci)
